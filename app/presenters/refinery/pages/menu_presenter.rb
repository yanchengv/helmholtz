...
module Refinery
  module Pages
    class MenuPresenter
      ...
      def render_html_without_child(pageid)
      ...
      def render_menu_items_without_child(menu_items,pageid)
        if menu_items.present?
          content_tag(list_tag) do
            menu_items.each_with_index.inject(ActiveSupport::SafeBuffer.new) do |buffer, (item, index)|
      ...
      def render_menu_item_without_child(menu_item, index,pageid)
        content_tag(list_item_tag, :class => menu_item_css(menu_item, index)) do
          buffer = ActiveSupport::SafeBuffer.new
          buffer << link_to(menu_item.title, context.refinery.url_for(menu_item.url))
          if menu_item.original_id != pageid
            buffer << render_menu_items(menu_item_children(menu_item))
          end
          buffer
        end
